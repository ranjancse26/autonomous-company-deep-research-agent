...
from agents.competitor_detection_agent import competitor_detection_agent
from utils.text_utils import count_tokens, truncate_text_to_tokens
from app.config import MODEL_MAX_TOKENS
import json

async def run_competitor_research(company: str):
    """
    Run research pipeline specifically for competitor detection.
    
    Args:
        company (str): Company name to research
        
    Returns:
        dict: Competitor data with list of competitor names
    """
    ...
    # Extract competitor information using specialized agent
    competitor_json = competitor_detection_agent(research_data)
    ...
    try:
        ...
        return {"error": "Failed to parse competitor data", "raw_response": competitor_json}