...
from agents.hiring_signals_agent import hiring_signals_agent
from utils.text_utils import count_tokens, truncate_text_to_tokens
from app.config import MODEL_MAX_TOKENS
import json

async def run_hiring_signals_research(company: str):
    """
    Run research pipeline specifically for hiring signals.
    
    Args:
        company (str): Company name to research
        
    Returns:
        dict: Hiring signals data with list of hiring signals
    """
    # Construct hiring-specific search queries
    queries = [
        f"{company} careers jobs",
        f"{company} hiring engineers"
    ]
    ...
    # Extract hiring signals using specialized agent
    hiring_json = hiring_signals_agent(research_data)
    
    # Return the hiring signals data
    try:
        hiring_data = json.loads(hiring_json)
        return hiring_data
    except json.JSONDecodeError:
        # If JSON parsing fails, return a structured error
        return {"error": "Failed to parse hiring signals data", "raw_response": hiring_json}